...
class GeminiApi {
  ...
  async handleListModels() {
    try {
      const remoteModelsMap = await this.auth.fetchAvailableModels().catch(() => null);
      ...
      for (const entry of entries) {
        ...
        const outputLimit = entry.outputTokenLimit || entry.outputTokenLimit || entry.maxOutputTokens;
        if (outputLimit) {
      ...
      if (models.length === 0) {
        models.push(
          ...
          { name: "models/gemini-3-flash", displayName: "Gemini 3 Flash", supportedGenerationMethods: ["generateContent", "streamGenerateContent"] },
          { name: "models/gemini-3.6-flash-high", displayName: "Gemini 3.6 Flash High", supportedGenerationMethods: ["generateContent", "streamGenerateContent"] }
        );
      }
